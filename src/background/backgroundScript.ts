// Api Stuffs here
import {
  factCheckWithGenerateQueries,
  factCheckWithoutGenerateQueries,
...
import {
  ...
  isFactCheckLoading,
} from "../utils/pop_up_storage/storage";
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === "UPDATE_FACT") {
    updateFactCheck(request.hypothesis)
      .then((response) => {
    ...
    if (info.selectionText) {
      ...
      if (!isMatch) {
        chrome.notifications.create({
          ...
          title: "Keyword Error",
          message:
            "The current selected text does not include any of the health keywords for this extension. Please select a text that includes health keywords to fact check.",
          priority: 2,
        });
        return;
      }
      ...
      if (!isHealthClaim) {
        ...
        if (error_value != "yes" || error_value != "no") {
          chrome.notifications.create({
            ...
            message:
              "I am sorry, but you have reached the daily limit or there is an error on the server. Please try again later.",
            priority: 2,
          });
          return;
        }
        chrome.notifications.create({
          ...
          message:
            "The text selected is not a health claim. Please select a health claim text to fact check.",
          priority: 2,
        });
        return;
      }
...
async function updateFactCheck(text) {
  ...
  if (loading) {
    chrome.notifications.create({
      ...
      title: "Error",
      message:
        "Please wait for the current fact check to finish before starting a new one.",
      priority: 2,
    });
    return;
  }
